...
const bestDefault = { buy: { price: -1 }, sell: { price: Number.MAX_VALUE } };

exports = module.exports = function (redis) {
  var marketFunc = () => {
    // Update the market data.
    LOG.info('Updating market data at ' + new Date().toString());
    async.eachLimit(config.get('dev') ? [REGIONS[1]] : REGIONS, 16, (region, cb) => {
          ...
          res.on('end', () => {
            ...
            async.eachSeries(body.items, (item, itemsCB) => {
              redis.get(prefix + ':' + item.type, (err, reply) => {
                ...
                // TODO Find a way to update pieces of market data.
